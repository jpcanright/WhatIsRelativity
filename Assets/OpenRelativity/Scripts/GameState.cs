...
public class GameState : MonoBehaviour
{
    ...
    public void Awake()
    {
        ...
        //J or don't playerVelocityVector = Vector3.zero;
        //playerVelocity = 0;
		//Set our constants
		MaxSpeed = MAX_SPEED;
    ...
    {
        playerVelocityVector = GameObject.FindGameObjectWithTag(Tags.mainCamera).GetComponent<getHeadsetVelocity>().velocity;
        ...
        if (!movementFrozen)
        {
            ...
            {
                playerVelocityVector = playerVelocityVector*1.0f;//.normalized * ((float)MaxSpeed-.01f);
            }
        ...
        /*
            ...
            {
                //J no GameObject.FindGameObjectWithTag(Tags.playerMesh).GetComponent<Rigidbody>().velocity = -1*(playerVelocityVector / (float)sqrtOneMinusVSquaredCWDividedByCSquared);
                //GameObject.FindGameObjectWithTag(Tags.mainCamera).GetComponent<Rigidbody>().velocity = -1 * (playerVelocityVector / (float)sqrtOneMinusVSquaredCWDividedByCSquared);
            }
            ...
            orientation = playerTransform.rotation;
